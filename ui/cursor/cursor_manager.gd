...
func ensure_playing_mode() -> void:
	if GameState.current_state == GameState.PLAYING:
		_exit_ui()


func enter_reading_mode() -> void:
	_enter_ui()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			...
			if top_piece and top_piece is ClickableArea2D and clicked_item == null:
				if _is_allowed_during_dialogue_gate(top_piece) or not _block_if_dialogue_gated():
					_start_click()
			elif top_piece and top_piece is DraggableArea2D and top_piece.can_drag and dragged_item == null:
				if _is_allowed_during_dialogue_gate(top_piece) or not _block_if_dialogue_gated():
					_start_drag()
			elif top_piece and top_piece is RotatableArea2D and rotated_item == null:
				if not _block_if_dialogue_gated():
					_start_rotate()
...
func _is_allowed_during_dialogue_gate(piece: Area2D) -> bool:
	return (
		piece.is_in_group("dialogue_click")
		or piece.is_in_group("note_click")
		or piece.is_in_group("dust_wipe")
	)


func _block_if_dialogue_gated() -> bool:
	return MessageManager != null and MessageManager.is_dialogue_gated()
