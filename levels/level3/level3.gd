...
const VICTORY_FIRST_LINE_PAUSE := 3.5
const VICTORY_LINE_PAUSE := 3.0
const VICTORY_END_PAUSE := 4.5
...
var is_puzzle_finished: bool = false
...
func _ready() -> void:
	...
	MessageManager.begin_dialogue_at_text("Ты еще тут? Мне показалось, что ты пропал", true)
...
func _complete_puzzle() -> void:
	if is_puzzle_finished:
		return
	is_puzzle_finished = true
	print("✅ Puzzle complete! TV настроен правильно!")
	_celebrate_success()
	puzzle_completed.emit()
	
	if MessageManager.begin_dialogue_at_text("Ну вот, кажется, что-то получилось", true):
		await get_tree().create_timer(VICTORY_FIRST_LINE_PAUSE).timeout
